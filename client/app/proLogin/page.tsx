...
function ProfessionalLogin() {
  ...
  const [passwordInputRef, setPasswordInputRef] = useState<string>("");
  ...
  useEffect(() => {
    ...
    if (
      (pathname === "/proLogin" && localStorage.getItem("pat-auth")) ||
      (localStorage.getItem("pro-auth"))
    ) {
  ...
  const logInForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("pro-auth", JSON.stringify(user));
      router.push("/games"); 
    } catch (error) {
